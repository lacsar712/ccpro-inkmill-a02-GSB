    ...
    <tbody>
      {#each rows as row}
        <tr>
          ...
          <td>
            {#if row.openWashOrderId && row.openWashOrderStatus}
              <span class="badge wash-order {row.openWashOrderStatus}">
                开放工单 #{row.openWashOrderId} · {bowlWashStatusLabel[row.openWashOrderStatus]}
              </span>
            {:else}
